...
<!-- <div class="modal-backdrop"> -->
  <div class="col-wrapper" :style="getWrapperHeight" style="margin-left: 5px; margin-right: 5px;">
    <ul class="wheel-list" :style="getListTop" ref="wheel">
      <li class="wheel-item" v-for="(item, index) in scrollValues" :key="index" :style="initWheelItemDeg(item.index)">{{item.value}}</li>
    </ul>
...
const singleDeg = deg
// 半圆下的内容条数
const space = Math.floor((360 / singleDeg) / 12)
export default {
  name: 'basicSelect',
  props: {
    values: {
  ...
  methods: {
    ...
    getSelectValue (move) {
      ...
      this.getRangeData(index)
      const pickvalue = this.scrollValues[1].value
...
<style scoped>
  ...
  .divider{
    ...
    width: 100%;
    left: 0;
    border-top: 1px solid rgb(26, 138, 190);
    border-bottom: 1px solid rgb(26, 138, 190);
  }
</style>
